...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *cashutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *cashutil.Tx, details *btcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount cashutil.Amount)
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*cashutil.Tx, error) {
	...
	transactions := make([]*cashutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = cashutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*cashutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return cashutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := cashutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance cashutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := cashutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []cashutil.Address, outpoints []*wire.OutPoint,
